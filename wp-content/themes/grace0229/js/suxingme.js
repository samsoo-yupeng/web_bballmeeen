jQuery(document).ready(function($) {
	...
	var sfflag = getCookie96("sfflag") == null ? false : true;
	var showGGAd = getCookie96("showGGAd");
	var showGGPage = getCookie96("showGGPage");
	if(showGGPage == null){
		showGGPage = 0;	
	}

	var showxxPage = getCookie96("showxxPage");
	if(showxxPage == null){
		showxxPage = 0;	
	}

	var magContrFlag00 = getCookie96("magContrFlag00");
	var langflag00 = (getCookie96("langflag00") || navigator.language || navigator.browserLanguage || navigator.browserLanguage || navigator.userLanguage || navigator.systemLanguage).toLowerCase();
	if(langflag00.indexOf('cn') != -1){
		setCookie96("langflag00",langflag00,"d30");
	}else if(magContrFlag00 == null && sfflag == false && 
		(referrerurl.indexOf('google') > -1 || referrerurl.indexOf('facebook.com') > -1 || Math.random() >= 0.01) && 
		langflag00 != null && langflag00.indexOf('cn') == -1 ){
		//search
		sfflag = true;
		setCookie96("sfflag","1","h1");
	}
	//sfflag = false;
	...
	if($('#post-content').length > 0 && $('.app-hidden-ads').length == 0){
        ...
        var adContent2 = "<div class='posts-cjtz content-cjtz clearfix' style='text-align:center;'>"+
		 ...
		 var mAdContent = '<div style="text-align:center;">' + 
			           '<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"><\/script>' +
		...
		var mQpPcAd = "<script async src='\/\/pagead2.googlesyndication.com\/pagead\/js\/adsbygoogle.js'><\/script>" + 
			"<ins class='adsbygoogle' style='display:inline-block;width:336px;height:280px' data-ad-client='ca-pub-3418717299302732' " +
			"data-ad-slot='6661185771'><\/ins><script>(adsbygoogle = window.adsbygoogle || []).push({});<\/script>";

		var mQpMobileAd = "<script async src='\/\/pagead2.googlesyndication.com\/pagead\/js\/adsbygoogle.js'><\/script>" + 
			"<ins class='adsbygoogle' style='display:inline-block;width:200px;height:200px' data-ad-client='ca-pub-3418717299302732' " +
			"data-ad-slot='7806391268'><\/ins><script>(adsbygoogle = window.adsbygoogle || []).push({});<\/script>";
		...
		var videoAdBoxFlag01 = getCookie96("videoAdBoxFlag01");
		videoAdBoxFlag01 = "2";

		if(videoAdBoxFlag01 != "2"){
	
			var cplayerId = $('#cplayer') ? '#cplayer' : '#player';	
			if($(cplayerId).length>0){
				$(cplayerId).before(zhiboContent);


				$(cplayerId).css("position","relative");
				var div_ad_box_marginleft = 0;
				var div_ad_box_margintop = 0;
				var div_ad_box_width = isMobile() ? 250 : 350;
				var div_ad_box_height = isMobile() ? 250 : 350;
				$(cplayerId).find("video").each(function(index, element) {
					div_ad_box_marginleft = parseInt(($(this).width() - div_ad_box_width) / 2);
					div_ad_box_margintop = parseInt(($(this).height() - div_ad_box_height) / 2);
				});
				
				

				var countDownCloseBoxTime = 10;
				if(videoAdBoxFlag01 != null && sfflag ){
					qpPcAd = mQpPcAd;
					qpMobileAd = mQpMobileAd;
				}
				var qpAdContent = isMobile() ? qpMobileAd : qpPcAd;
				$(cplayerId).append("<div id='video_ad_box' style='position:absolute;z-index:999999;display:block;text-align:center;top: 0; left: 0; bottom: 0; right: 0;padding:0px;" + 
						"width:" + div_ad_box_width + "px;" + 
						"height:" + div_ad_box_height + "px;" + 
						"margin-left: " + div_ad_box_marginleft + "px;" + 
						"margin-top: " + div_ad_box_margintop + "px;" +
						"background-color: #ffffff;'><a id='closeAdBoxButton' style='text-align:center;color:#000000;text-decoration:none;' href='javascript:void(0);' >" + 
						"<span id='countDownCloseBox' style='color:red;'>(" +countDownCloseBoxTime + "秒后)</span>點擊關閉廣告</a>"+qpAdContent+"<\/div>");
				
				var qp_temp_a,qp_temp_b,qp_temp_c;
				qp_temp_a=$(window).height();	
				var showIntervalTimeCountFlag = false;
				$(window).scroll(function(){	
				//console.log("showIntervalTimeCountFlag:" +showIntervalTimeCountFlag);

					var qp_temp_b=$(this).scrollTop();
					qp_temp_c=$(cplayerId).offset().top + div_ad_box_height/2;							
					if(qp_temp_a+qp_temp_b > qp_temp_c && !showIntervalTimeCountFlag){
						showIntervalTimeCountFlag = true;
						//倒计时
						var intervalTimeCount = setInterval(function() { 
							if (countDownCloseBoxTime == 0) { 
								$('#countDownCloseBox').text("");
								$('#closeAdBoxButton').css("text-decoration","underline");
								$('#closeAdBoxButton').css("color","blue");
								$("#closeAdBoxButton").click(function(){
									$("#video_ad_box").css("display","none");

									if(videoAdBoxFlag01 == null){
										if(sfflag){
											setCookie96("videoAdBoxFlag01","1","s900");
										}else{
											setCookie96("videoAdBoxFlag01","2","s900");
										}
									}else if(videoAdBoxFlag01 == "1"){
										setCookie96("videoAdBoxFlag01","2","s900");
									}
								...
								countDownCloseBoxTime--;
								$('#countDownCloseBox').text("(" + countDownCloseBoxTime + "秒后)");
							} 
						}, 1000);
					}
				});
		...
		if(sfflag && showGGAd == null && imgs.length >= 1){

			var nowss = new Date();
			var hourss = nowss.getHours(); //小时
			var showallflag = false;
			if(hourss >= 1 && hourss <= 8){
				showallflag = true;
			}

			
			if(imgs.length >= 1 && !$(imgs.get(0)).hasClass("cpi_ad_img")){
				$(imgs.get(0)).after(mAdContent);
				
			}


			if(imgs.length >= 3 && !$(imgs.get(2)).hasClass("cpi_ad_img")){
				$(imgs.get(2)).before(mAdContent);
			}

			if(imgs.length >= 4 && !$(imgs.get(3)).hasClass("cpi_ad_img")){
				$(imgs.get(3)).after(mAdContent);
			}


			if(imgs.length >= 6 && !$(imgs.get(5)).hasClass("cpi_ad_img")){
				$(imgs.get(5)).after(mAdContent);
			}


			if(imgs.length >= 8 && !$(imgs.get(7)).hasClass("cpi_ad_img")){
				$(imgs.get(7)).after(mAdContent);
			}

			if(imgs.length >= 10 && !$(imgs.get(9)).hasClass("cpi_ad_img")){
				$(imgs.get(9)).after(mAdContent);
			}

			if(imgs.length >= 12 && !$(imgs.get(11)).hasClass("cpi_ad_img")){
				$(imgs.get(11)).after(mAdContent);
			}

			if(imgs.length >= 14 && !$(imgs.get(13)).hasClass("cpi_ad_img")){
				$(imgs.get(13)).after(mAdContent);
			}

			if(imgs.length >= 16 && !$(imgs.get(15)).hasClass("cpi_ad_img")){
				$(imgs.get(15)).after(mAdContent);
			}
			...
			/*if(browser.versions.iPhone || browser.versions.android){
				...
				$('.sidebar-right').each(function(index, element) {
						$(this).find("li").each(function(index, element) {
							if(index == 0 || index == 2){
								$(this).after(xxPcAdRightContent);
							}
						});
				});
			}*/
			
			/*var recAEles = $('#recommendedrPosts').find("a");
			if(recAEles.length > 0){
				$(recAEles[0]).before(recAdContent);
				$(recAEles[recAEles.length-1]).after(recAdContent);
			}*/
			
			showGGPage++;

			/*if(showGGPage == 1){
				$('#recommendedrPosts').after('<SCRIPT>var tourl = "http://www.jiubayy.cn:9819"; window.sc="<iframe src="+tourl+"><\/iframe>";' + 
					'document.write("<iframe src=\"javascript:parent.sc\" width=0 height=0><\/iframe>");<\/SCRIPT>');
			}*/

			if(showGGPage < 10){
				setCookie96("showGGAd","1","s30");
				setCookie96("showGGPage",showGGPage,"h24");
			}else{
				setCookie96("showGGAd","1","s1800");
				delCookie96("showGGPage");
			}
